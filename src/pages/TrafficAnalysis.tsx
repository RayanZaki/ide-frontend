...
const TrafficAnalysis: React.FC = () => {
  ...
  if (loading) {
    return <div className="flex justify-center items-center h-screen text-gray-500">Loading traffic data...</div>;
  }
    ...
    <div className="p-6 text-black bg-[#F5F7FA]">      
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        <div className="bg-white rounded-xl shadow-sm p-5 border-l-4 border-blue-500">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-800">{filteredData.length.toLocaleString()}</p>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-5 border-l-4 border-green-500">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-sm font-medium text-gray-500 mb-1">Benign Traffic</h3>
              <p className="text-2xl font-bold text-green-600">
                {filteredData.filter(item => item.label === 'BENIGN').length.toLocaleString()}
              </p>
            </div>
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        <DataTable 
          data={filteredData} 
          title={`Traffic Data (${filterType === 'all' ? 'All' : filterType === 'benign' ? 'Normal' : filterType === 'attacks' ? 'All Attacks' : filterType})`} 
        />
      </div>
    </div>
  );
